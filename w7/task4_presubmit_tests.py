...
from task4_optimization import *
...
def test_least_squares_oracle():
    ...
    oracle = LeastSquaresOracle(matvec_Ax, matvec_ATx, b)
...
def test_lasso_prox_oracle():
    A = np.eye(2)
    b = np.array([1.0, 2.0])
    oracle = create_lasso_prox_oracle(A, b, regcoef=1.0)
...
def test_proximal_gm_prototype():
    method = proximal_gradient_method

    A = np.array([[1.0, 2.0], [3.0, 4.0]])
    b = np.array([1.0, 2.0])
    oracle = create_lasso_prox_oracle(A, b, regcoef=2.0)
...
def test_proximal_gm_one_step():
    ...
    A = np.eye(2)
    b = np.array([1.0, 0.0])
    oracle = create_lasso_prox_oracle(A, b, regcoef=0.0)
    x_0 = np.zeros(2)

    x_star, status, hist = proximal_gradient_method(
...
def test_proximal_gm_nonsmooth():
    # Minimize ||x||_1.
    oracle = create_lasso_prox_oracle(np.zeros([2, 2]),
                                              np.zeros(2),
                                              regcoef=1.0)
